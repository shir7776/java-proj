...
public class GeometriesTest {
    ...
    public void findIntsersections() {
        ...
        result = geometries.findIntsersections(new Ray(new Vector(0,0, -1), new Point3D(0, 6, 1)));
        assertEquals("Wrong number of points", 1, result.size());

        //TC4: several geometriesget intersected (but not all of them)
        result = geometries.findIntsersections(new Ray(new Vector(0,0, -1), new Point3D(0, 6, 4)));
        assertEquals("Wrong number of points", 2, result.size());

        //TC5: all geometries get intersected
        result = geometries.findIntsersections(new Ray(new Vector(0,0, -1), new Point3D(0.5, 0.5, 4)));
